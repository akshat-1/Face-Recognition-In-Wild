...
def setup_ssh_multiplexing():
    ...
    log(f"SSH ControlMaster socket directory initialized: {SOCKET_DIR}")
...
def verify_remote_scratch(scratch_dest):
    """
    Verifies transferred files on AQUA cluster scratch directory after upload.
    """
...
def main():
    parser = argparse.ArgumentParser(description="High-Speed Zero-Disk Streaming Transfer with Detailed Real-Time Logging")
    ...
    log(f"Target AQUA Destination: {AQUA_USER}@{AQUA_HOST}:{args.scratch_dest}")
    log("==========================================================================")
    ...
    if check_command("rclone"):
        try:
            ...
            verify_remote_scratch(scratch_dest)
